...
scenario (`sweep_single_share_highest_plus10`) and computed on the
category-netted Δq, pooled across store-weeks as a single micro-average:

* **own = signed WMPE** on the category-netted FOCAL Δq. Signed because the
  own axis is an identification / bias test — endogeneity bias is
...
* **substitution = unsigned WAPE** (L1 / total variation) on the category-netted
  ...
  geometry-blind. No direction score, no proximity weights, no
  renormalization to sum-to-1. This axis is honest accuracy and is not
  required to separate naive from sophisticated.

Netting: each side's residual is `Δq − ΔM·s_base` with `ΔM = Σ_j Δq` the
row-sum of THAT side's own Δq and `s_base` the baseline-unit share. This
makes both axes independent of the category/incidence magnitude the headline
does not grade (a no-change or pure-proportional-contraction prediction nets
to 0; a right-substitution-wrong-category prediction nets to the true
substitution). The focal is INCLUDED in the ΔM row-sum.
...
def decomposed_headline(frame: pd.DataFrame, focal: str) -> dict[str, Any]:
    ...
    * ``substitution_wape`` — UNSIGNED WAPE (raw-mass L1) on the category-netted
      ...
      pooled denominator ``Σ_sw Σ_{k≠f} |rt_k|`` is 0.

    Both on category-netted residuals ``Δq − ΔM·s_base``. Numerators and
    denominators are pooled and divided ONCE (micro-average); the four
    ...
    for (sw_store, sw_week), group in work.groupby(["store_id", "week"], sort=True):
        ...
        # (= Σ_j Δq, focal INCLUDED), so the ungraded category/incidence
        # magnitude cancels.
        ...
        # mass, geometry-blind. No direction score, no proximity weights, no
        # per-store-week ratio, no renormalization.
        is_comp = ~is_focal
        if is_comp.any():
