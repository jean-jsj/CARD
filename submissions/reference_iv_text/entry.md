# Reference model: `iv_text`

One corner of the benchmark's 2x2 reference grid (with/without the released
instruments x with/without the product text). This corner uses both the released instruments and the product text.

- **Model.** Each cell's own demand family (the log-linear demand system or the
  discrete-choice system), fit from the `public/` files alone, with two-stage least squares with text-disciplined substitution structure.
...
- **Predictions.** Full submission-format CSVs are hosted with the dataset
  (Hugging Face `jean-jsj/CARD`, `reference/iv_text/`) — the per-cell counterfactual
