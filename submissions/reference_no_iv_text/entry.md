# Reference model: `no_iv_text`

One corner of the benchmark's 2x2 reference grid (with/without the released
instruments x with/without the product text). This corner uses the product text (embedding distances discipline the substitution structure) but not the released instruments.
...
- **Predictions.** Full submission-format CSVs are hosted with the dataset
  (Hugging Face `jean-jsj/CARD`, `reference/no_iv_text/`) — the per-cell Layer-3
